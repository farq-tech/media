...
<body>
    ...
    <aside>
        ...
        <div class="filter-section">
            <span class="filter-label">Media / Content Type</span>
            <div class="filter-group" id="media-type-filters">
                <button class="filter-btn active" data-type="" onclick="setMediaTypeFilter('')">All</button>
                <button class="filter-btn" data-type="photo" onclick="setMediaTypeFilter('photo')">Photo</button>
                <button class="filter-btn" data-type="video" onclick="setMediaTypeFilter('video')">Video</button>
                <button class="filter-btn" data-type="exterior" onclick="setMediaTypeFilter('exterior')">Exterior</button>
                <button class="filter-btn" data-type="interior" onclick="setMediaTypeFilter('interior')">Interior</button>
                <button class="filter-btn" data-type="license" onclick="setMediaTypeFilter('license')">License</button>
                <button class="filter-btn" data-type="menu" onclick="setMediaTypeFilter('menu')">Menu</button>
                <button class="filter-btn" data-type="other" onclick="setMediaTypeFilter('other')">Other</button>
            </div>
        </div>
...
<script>
    ...
    function renderGrid() {
        ...
        function matchesTypeFilter(href, filterType) {
            ...
            const userTags = JSON.parse(localStorage.getItem(LOCAL_TAGS_KEY) || '{}');
            return (userTags[href] || []).includes(filterType) ||
                h.includes(filterType + '_') || h.includes(filterType + '-') || h.includes(filterType + '.');
        }

        container.innerHTML = filtered.map(p => {
            let mediaOnly = p.mediaLinks.filter(m => !m.isPublic && m.href && m.href !== '#');
            if (activeMediaTypeFilter && activeMediaTypeFilter !== 'photo' && activeMediaTypeFilter !== 'video') {
                mediaOnly.sort((a, b) => {
                    const aMatch = matchesTypeFilter(a.href, activeMediaTypeFilter) ? 0 : 1;
                    const bMatch = matchesTypeFilter(b.href, activeMediaTypeFilter) ? 0 : 1;
    ...
    function setMediaTypeFilter(t) {
        activeMediaTypeFilter = t;
        document.querySelectorAll('.filter-btn[data-type]').forEach(btn => btn.classList.toggle('active', btn.dataset.type === t));
        renderGrid();
    }
